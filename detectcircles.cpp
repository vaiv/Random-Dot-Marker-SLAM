...
//    cv::GaussianBlur(gray,gray,cvSize(9,9),0,0);
//    return gray;
//}
...
//    //cv::adaptiveThreshold(binaryMask,binaryMask,200,cv::ADAPTIVE_THRESH_GAUSSIAN_C,cv::THRESH_BINARY,11,2);
...
//       inImage =  cvScalar(255,255,255) - inImage;
     ...
     cv::Mat im_with_keypoints;
                       ...
                       for( vector<cv::KeyPoint>::iterator it = keypoints.begin(); it!= keypoints.end(); it++ )
                               {
                           cv::circle(im_with_keypoints, it->pt, 3, cv::Scalar(0,0,255));
                           cv::Vec3f Circle;
                                   Circle[0] = it->pt.x;
                                   Circle[1] = it->pt.y;
                                   Circle[2] = (it->size)/2;
                       ...
                       //cout<<"Loop Exit"<<endl;
                      cv::imshow("results", im_with_keypoints);
                             cv::waitKey(1);
}
